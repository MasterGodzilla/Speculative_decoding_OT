...
class SpecInferTreeTest(Tree):
    ...
    def collective_grow_static(self, idx_list :torch.LongTensor, n_branch_list :list[int], benchmark=False):
        ...
        new_tokens_set = sampling_q.multinomial(num_samples=max_branch, replacement=True)
    ...
    def accept_step(self, parent_id :int) ->ChildrenAccept:
        ...
        for idx, pos in enumerate(children):
            ...
            if p[token] >= r * q[token]:
                ...
                p = get_residual(p, q)
                # draft_logits[token] = -torch.inf # comment out since RSS with replacement do not need to mask out the token
